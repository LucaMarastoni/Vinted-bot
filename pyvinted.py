...
class Vinted:
    ...
    def getHeaders(self):
        return {
            ...
            "accept-language": "it,it-IT;q=0.9,en;q=0.8,it-IT;q=0.7",
            "cache-control": "no-cache",
            "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36"
            }

    def InitVintedSession(self):
        self.session.get('https://www.google.it/') # Prepare session Fingerprint
        self.session.get('https://vinted.it/', headers=self.getHeaders()) # Get main vinted cookies
    ...
    def search(self, search_text, order_type="", size=[], max_price=999, page=1, max_items_per_page=24):
        ...
        sz_str = ""
        for sz in size:
            sz_str += f"&size_ids[]={self.sizes[sz]}"
        ...
        url = self.base_url + "/catalog/items" + f"?search_text={search_text}&catalog_ids={f'&price_to={max_price}' if max_price > 0 else ''}&currency={self.currency}&color_ids=&brand_ids={sz_str}&material_ids=&video_game_rating_ids=&status_ids=&order={order_type}&page={page}&per_page={max_items_per_page}"
        # print(url)
